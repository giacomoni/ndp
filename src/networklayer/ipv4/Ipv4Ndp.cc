...
void Ipv4Ndp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        icmp = getModuleFromPar<IcmpNdp>(par("icmpModule"), this);
...
void Ipv4Ndp::handleRegisterService(const Protocol& protocol, cGate *out, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterService");
}

void Ipv4Ndp::handleRegisterProtocol(const Protocol& protocol, cGate *in, ServicePrimitive servicePrimitive)
{
...
void Ipv4Ndp::refreshDisplay() const
{
...
void Ipv4Ndp::handleRequest(Request *request)
{
...
void Ipv4Ndp::handleMessageWhenUp(cMessage *msg)
{
...
void Ipv4Ndp::preroutingFinish(Packet *packet)
{
...
void Ipv4Ndp::handlePacketFromHL(Packet *packet)
{
...
void Ipv4Ndp::datagramLocalOut(Packet *packet)
{
...
void Ipv4Ndp::routeUnicastPacket(Packet *packet)
{
...
void Ipv4Ndp::routeUnicastPacketFinish(Packet *packet)
{
...
void Ipv4Ndp::routeLocalBroadcastPacket(Packet *packet)
{
...
void Ipv4Ndp::reassembleAndDeliver(Packet *packet)
{
    EV_INFO << "Delivering " << packet << " locally.\n";
...
void Ipv4Ndp::fragmentPostRouting(Packet *packet)
{
...
void Ipv4Ndp::fragmentAndSend(Packet *packet)
{
    const InterfaceEntry *destIE = ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
    Ipv4Address nextHopAddr = getNextHop(packet);
...
void Ipv4Ndp::encapsulate(Packet *transportPacket)
{
    ...
    ipv4Header->setProtocolId((IpProtocolId)ProtocolGroupNdp::ipprotocol.getProtocolNumber(transportPacket->getTag<PacketProtocolTag>()->getProtocol()));
...
void Ipv4Ndp::sendDatagramToOutput(Packet *packet)
{
...
void Ipv4Ndp::arpResolutionCompleted(IArp::Notification *entry)
{
...
void Ipv4Ndp::arpResolutionTimedOut(IArp::Notification *entry)
{
    if (entry->l3Address.getType() != L3Address::IPv4)
        return;
    auto it = pendingPackets.find(entry->l3Address.toIpv4());
...
MacAddress Ipv4Ndp::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)
{
...
void Ipv4Ndp::sendPacketToNIC(Packet *packet)
{
...
void Ipv4Ndp::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    NetfilterBase::registerHook(priority, hook);
}

void Ipv4Ndp::unregisterHook(INetfilter::IHook *hook)
{
    Enter_Method("unregisterHook()");
    NetfilterBase::unregisterHook(hook);
}

void Ipv4Ndp::dropQueuedDatagram(const Packet *packet)
{
...
void Ipv4Ndp::reinjectQueuedDatagram(const Packet *packet)
{
...
INetfilter::IHook::Result Ipv4Ndp::datagramPreRoutingHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ndp::datagramForwardHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ndp::datagramPostRoutingHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
void Ipv4Ndp::handleStartOperation(LifecycleOperation *operation)
{
    start();
}

void Ipv4Ndp::handleStopOperation(LifecycleOperation *operation)
{
    // TODO: stop should send and wait pending packets
    stop();
}

void Ipv4Ndp::handleCrashOperation(LifecycleOperation *operation)
{
    stop();
}
...
INetfilter::IHook::Result Ipv4Ndp::datagramLocalInHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ndp::datagramLocalOutHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
void Ipv4Ndp::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();
...
void Ipv4Ndp::sendIcmpError(Packet *origPacket, int inputInterfaceId, IcmpType type, IcmpCode code)
{
