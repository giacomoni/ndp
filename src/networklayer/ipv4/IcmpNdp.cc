...
Define_Module(IcmpNdp);

void IcmpNdp::initialize(int stage)
{
...
void IcmpNdp::handleMessage(cMessage *msg)
{
...
void IcmpNdp::sendErrorMessage(Packet *packet, int inputInterfaceId, IcmpType type, IcmpCode code)
{
...
bool IcmpNdp::possiblyLocalBroadcast(const Ipv4Address& addr, int interfaceId)
{
    if ((addr.getInt() & 1) == 0)
        return false;
...
void IcmpNdp::processICMPMessage(Packet *packet)
{
    if (!verifyCrc(packet)) {
        ...
        case ICMP_PARAMETER_PROBLEM: {
            ...
            if (transportProtocol == IP_PROT_ICMP) {
                ...
                if (transportProtocols.find(transportProtocol) == transportProtocols.end()) {
                    ...
                    dispatchProtocolReq->setProtocol(ProtocolGroupNdp::ipprotocol.getProtocol(transportProtocol));
...
void IcmpNdp::errorOut(Packet *packet)
{
    delete packet;
}

void IcmpNdp::processEchoRequest(Packet *request)
{
...
void IcmpNdp::sendToIP(Packet *msg, const Ipv4Address& dest)
{
    msg->addTagIfAbsent<L3AddressReq>()->setDestAddress(dest);
    sendToIP(msg);
}

void IcmpNdp::sendToIP(Packet *msg)
{
...
void IcmpNdp::handleRegisterService(const Protocol& protocol, cGate *out, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterService");
}

void IcmpNdp::handleRegisterProtocol(const Protocol& protocol, cGate *in, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterProtocol");
    if (!strcmp("transportIn", in->getBaseName())) {
        int protocolNumber = ProtocolGroupNdp::ipprotocol.findProtocolNumber(&protocol);
        if (protocolNumber != -1)
            transportProtocols.insert(protocolNumber);
    }
}

void IcmpNdp::insertCrc(CrcMode crcMode, const Ptr<IcmpHeader>& icmpHeader, Packet *packet)
{
...
bool IcmpNdp::verifyCrc(const Packet *packet)
{
