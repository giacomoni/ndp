...
#ifndef NETWORKLAYER_IPV4_IGMPV2NDP_H_
#define NETWORKLAYER_IPV4_IGMPV2NDP_H_
...
class INET_API Igmpv2Ndp : public cSimpleModule, public IProtocolRegistrationListener, public cListener
{
  protected:
    enum RouterState {
        IGMP_RS_INITIAL,
    ...
    struct HostGroupData
    {
        ...
        HostGroupData(Igmpv2Ndp *owner, const Ipv4Address& group);
    ...
    struct RouterGroupData
    {
        ...
        RouterGroupData(Igmpv2Ndp *owner, const Ipv4Address& group);
    ...
    struct HostInterfaceData
    {
        ...
        HostInterfaceData(Igmpv2Ndp *owner);
        virtual ~HostInterfaceData();
        friend inline std::ostream& operator<<(std::ostream& out, const Igmpv2Ndp::HostInterfaceData& entry)
        {
            for(auto& g : entry.groups) {
                out << "(groupAddress: " << g.second->groupAddr << " ";
                out << "hostGroupState: " << Igmpv2Ndp::getHostGroupStateString(g.second->state) << " ";
    ...
    struct RouterInterfaceData
    {
        ...
        RouterInterfaceData(Igmpv2Ndp *owner);
        virtual ~RouterInterfaceData();
        friend inline std::ostream& operator<<(std::ostream& out, const Igmpv2Ndp::RouterInterfaceData& entry)
        {
            out << "routerState: " << Igmpv2Ndp::getRouterStateString(entry.igmpRouterState) << " ";
            ...
            if(entry.groups.empty())
                ...
                for(auto& g : entry.groups) {
                    out << "(groupAddress: " << g.second->groupAddr << " ";
                    out << "routerGroupState: " << Igmpv2Ndp::getRouterGroupStateString(g.second->state) << " ";
